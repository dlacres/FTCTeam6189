...
task main()
{
  ...
  ClearTimer(T2);
	...
	int	bias=SensorValue[gyro];
	wait10Msec(20);
	bias=(SensorValue[gyro]+bias);
	wait10Msec(20);
	bias=(SensorValue[gyro]+bias)/3;
	...
	while(true)
	{
	  ...
	  jstickX = jstickX-((SensorValue[gyro]-bias)/2);
	  writeDebugStreamLine("sensor=%d",SensorValue[gyro]);
