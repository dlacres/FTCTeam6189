...
ForwardDistReset((tMotor)rtMotor, (tMotor)ltMotor);
  ...
  waitForStart(); // Wait for the beginning of autonomous phase.
	...
	while(true){
		...
		long  robotDist = nMotorEncoder[rtBack] + nMotorEncoder[ltBack];
		long  robotDir  = nMotorEncoder[ltBack] - nMotorEncoder[rtBack];
...
//		rightmotors=speedCmd+dirCmd; // for using back encoders
//		leftmotors=speedCmd-dirCmd;
/*		dirCmd = 0;
		speedCmd = 30;
		*/
		rightmotors=speedCmd-dirCmd; // for using front encoders
		leftmotors=speedCmd+dirCmd;
		...
		if (iFrameCnt==0)
			writeDebugStreamLine("i,pthIdx,rbtDist,irval,spdCmd,IR,state,Degrees, DirSpd");
		writeDebugStreamLine("%3i,%3i,%4i,%4i,%3i,%3i,%3i, %3i, %3i",iFrameCnt,pathIdx,distInches,IRval,speedCmd,SensorValue[IR],state, dirDegrees, dirCmd);
		//--------------------------Robot Code--------------------------//
